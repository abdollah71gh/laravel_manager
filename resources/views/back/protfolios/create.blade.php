@extends('back.index')
@section('title')
پنل نمونه کارها - ایجاد نمونه کارها
@endsection
@section('content')
            ...
            <div class="row page-title-header">
                <div class="col-12">
                    <div class="page-header">
                        <h4 class="page-title">   ایجاد نمونه کارها </h4>
                    </div>
                </div>
                            ...
                            <form method="post" action="{{route('admin.portfolios.store')}}">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('tag') is-invalid @enderror"
                                           name="tag"
                                           placeholder=" لطفا تگ خود خود را وارد نمایید" value="{{old('tag')}}">
                                    @error('tag')
                                    <div class="alert alert-danger">{{$message}}</div>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('link') is-invalid @enderror"
                                           name="link"
                                           placeholder=" لینک خود را وارد نمایید" value="{{old('link')}}">
                                    @error('link')
                                    <div class="alert alert-danger">{{$message}}</div>
                                    @enderror
                                </div>
