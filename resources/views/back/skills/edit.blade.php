@extends('back.index')
@section('title')
    پنل ویرایش مهارت ها - ویرایش  مهارت ها
@endsection
@section('content')
            ...
            <div class="row page-title-header">
                <div class="col-12">
                    <div class="page-header">
                        <h4 class="page-title">   ویرایش  مهارت ها </h4>
                    </div>
                </div>
                            ...
                            <form method="post" action="{{route('admin.skills.update',$skill->id)}}">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                                           name="name"
                                           placeholder="نام خود را وارد نمایید" value="{{$skill->name}}">
                                    @error('name')
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea type="text"  rows="6" class="form-control @error('body') is-invalid @enderror"
                                              name="body"
                                              placeholder="توضیحات مربوطه خود را وارد نمایید">{{$skill->body}}</textarea>
                                    @error('body')
                                    <div class="alert alert-danger">{{$message}}</div>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('title') is-invalid @enderror"
                                           name="title"
                                           placeholder=" لطفا عنوان  خود را وارد نمایید" value="{{$skill->title}}">
                                    @error('title')
                                    <div class="alert alert-danger">{{$message}}</div>
                                    @enderror
                                </div>
                                ...
                                <a href="{{route('admin.skills')}}" class="btn btn-info btn-block rounded-pill">بازگشت</a>
                            </form>
                        </div>
                    </div>
                </div>
